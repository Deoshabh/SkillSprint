...
import { SquarePen, FileText, Image as ImageIcon, PlusCircle, Save, Trash2, Edit2, Loader2, List, Eye, EyeOff, Upload } from 'lucide-react';
        ...
        <Card className="shadow-xl">
          ...
          <CardContent className="space-y-4">
            {isEditingNote || currentNote ? (
              <div className="p-1 border rounded-lg bg-muted/30 shadow-inner">                  <div className="space-y-3 p-3">
                      ...
                      <DialogContent className="sm:max-w-md">
                        ...
                        <ImageUpload
                          ...
                          onImageSelected={(imageUrl) => {
                            const imageMarkdown = `\n\n![Image](${imageUrl})\n\n`;
                            setNoteBodyInput(prev => prev + imageMarkdown);
                    ...
                    <div key={note.id} className={cn(
                      ...
                      <div className="flex justify-between items-start">
                        ...
                        <div className="flex gap-1 flex-shrink-0 ml-2">
                           <Button variant="ghost" size="icon" onClick={() => handleSelectNote(note)} title="Edit Note" aria-label={`Edit note ${note.title}`}><Edit2 className="h-4 w-4" aria-hidden="true" /></Button>
                           <AlertDialog>
                            <AlertDialogTrigger asChild>
                              <Button variant="ghost" size="icon" title="Delete Note" aria-label={`Delete note ${note.title}`}><Trash2 className="h-4 w-4 text-destructive" aria-hidden="true" /></Button>
                            </AlertDialogTrigger>
                            <AlertDialogContent>
                 ...
                 <div className="mt-4">
                    ...
                    {sketches.sort((a,b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime()).map(sketch => (
                        ...
                        <div className="flex justify-between items-start">
                            ...
                            <div className="flex gap-1 flex-shrink-0 ml-2">
                                <Button variant="ghost" size="icon" onClick={() => handleSelectSketch(sketch)} title="Edit Sketch" aria-label={`Edit sketch ${sketch.title}`}><Edit2 className="h-4 w-4" aria-hidden="true" /></Button>
                                <AlertDialog>
                                    <AlertDialogTrigger asChild>
                                    <Button variant="ghost" size="icon" title="Delete Sketch" aria-label={`Delete sketch ${sketch.title}`}><Trash2 className="h-4 w-4 text-destructive" aria-hidden="true" /></Button>
                                    </AlertDialogTrigger>
                                    <AlertDialogContent>

...
export default function PlannerPage() {
  ...
  const fetchDailyPlans = async () => {
    try {
      const response = await fetch('/api/user/daily-plans');
      if (response.ok) {
        const data = await response.json();
        const dailyPlans = data.dailyPlans || {};
        setTasksForSelectedDate(dailyPlans[selectedDateKey] || []);
      }
    } catch (error) {
  ...
  const saveDailyPlans = async (updatedDailyPlans: DailyPlans) => {
    try {
      const response = await fetch('/api/user/daily-plans', {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ dailyPlans: updatedDailyPlans })
      });
      ...
      toast({ 
        title: "Save Error", 
        description: "Failed to save your daily plans. Please try again.",
        variant: "destructive"
      });
    }
  };
  ...
  const handleSaveTask = async () => {
    ...
    let updatedDailyPlans: DailyPlans = {};
    
    // Fetch current daily plans from the database to ensure we have the latest data
    try {
      const response = await fetch('/api/user/daily-plans');
      if (response.ok) {
        const data = await response.json();
        updatedDailyPlans = data.dailyPlans || {};
      }
    } catch (error) {
      console.error('Failed to fetch current daily plans:', error);
    }
    ...
    await saveDailyPlans(updatedDailyPlans);
    setTasksForSelectedDate(updatedDailyPlans[selectedDateKey] || []);

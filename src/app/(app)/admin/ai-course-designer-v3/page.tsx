...
import { useState, type ChangeEvent, type FormEvent, useCallback, useRef } from 'react';
  ...
  // Mock AI functions for build time - these should be replaced with actual imports in production
  const mockAIFunction = async (input: any) => {
    await new Promise(resolve => setTimeout(resolve, 1000)); // Simulate API delay
    return {
      courseSyllabus: 'Mock syllabus content for build time',
      scheduleText: 'Mock schedule for build time',
      subtopics: ['Topic 1', 'Topic 2', 'Topic 3'],
      practiceTask: 'Mock practice task for build time',
      videos: [],
      structuredContent: {
  ...
  // These should be actual AI imports in production:
  // import { autoGenerateCourseSyllabus } from '@/ai/flows/auto-generate-course-syllabus';
  // etc.
  const autoGenerateCourseSyllabus = mockAIFunction;
  const suggestModuleSubtopics = mockAIFunction;
  const suggestModulePracticeTask = mockAIFunction;
  const findYoutubeVideosForModule = mockAIFunction;
  const generateCourseSchedule = mockAIFunction;
  const extractAndAnalyzeContent = mockAIFunction;

  // Define input types for AI functions
  interface AutoGenerateCourseSyllabusInput {
    courseTopic: string;
    targetAudience: string;
    learningObjectives: string;
    desiredNumberOfModules: number;
  }

  interface SuggestModuleSubtopicsInput {
    moduleTitle: string;
  ...
  interface SuggestModulePracticeTaskInput {
    moduleTitle: string;
    moduleDescription: string;
    subtopics: string[];
    courseTopic: string;
  ...
  interface FindYoutubeVideosInput {
    moduleTitle: string;
    moduleDescription: string;
    preferredLanguage: string;
  }

  interface GenerateCourseScheduleInput {
    courseTitle: string;
    moduleTitles: string[];
    estimatedCourseDurationWeeks: number;
  ...
  interface ExtractContentInput {
    ...
    contentUrl: string;
    rawContent: string;
    contentTitle?: string;  extractionGoals: string[];
  }
  ...
  const fileInputRef = useRef<HTMLInputElement>(null);
    ...
    const handleGenerateCourse = async () => {
      ...
      try {
        ...
        updateProgress(1, 100, 'completed');      // Stage 3: Module Enhancement
        updateProgress(2, 0);
        const enhancedModules: Module[] = [];
        
        for (let i = 0; i < parsedModules.length; i++) {
          const module = parsedModules[i];
          updateProgress(2, (i / parsedModules.length) * 80);        // Enhance module with AI suggestions
          ...
          for (let i = 0; i < enhancedModules.length; i++) {
            ...
            updateProgress(3, (i / enhancedModules.length) * 100);
              ...
              const videoResult = await findYoutubeVideosForModule(videoInput);
                ...
                if (videoResult.videos[0] && (videoResult.videos[0] as any).youtubeEmbedUrl) {
                  module.contentUrl = (videoResult.videos[0] as any).youtubeEmbedUrl;
                }
              }
            } catch (error) {
        ...
        updateProgress(5, 0);
          ...
          instructor: user?.fullName || user?.firstName || 'AI Generated',

...
export const getCoursesForReview = (): Course[] => {
  return placeholderCourses.filter(course => course.status === 'pending');
};
...
export const updateCourseStatus = (courseId: string, newStatus: Course['status']): boolean => {
  ...
  if (courseIndex > -1) {
    const course = placeholderCourses[courseIndex] as any;
...
export const submitCourseForReview = (courseId: string): boolean => {
  ...
  if (courseIndex > -1) {
    ...
    course.submittedDate = new Date().toISOString();
    course.lastModified = new Date().toISOString();
...
export const saveOrUpdateCourse = (courseData: Partial<Course> & { authorId: string }): Course | null => {
  ...
  if (existingCourseIndex > -1) {
    ...
    const newCourse: Course = {
      ...
      lastModified: new Date().toISOString(),
      submittedDate: courseData.status === 'pending' ? new Date().toISOString() : undefined,
